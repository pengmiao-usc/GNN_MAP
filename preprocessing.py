...
def build_graph(past_addr, past_page, feature, labels):
    edge_list=[]
    for i in range(cf.LOOK_BACK):
        edge_list.append([i,i+1])
        for j in range(i+1,cf.LOOK_BACK+1):
            if past_page[i] == past_page[j]:
                edge_list.append([i,j])
    #print(edge_list)
    return Data(x=torch.Tensor(feature), 
                edge_index=torch.tensor(edge_list,dtype=torch.long).T.contiguous(),
                t=torch.Tensor(labels))

def preprocessing_gnn(data):
    print("preprocessing for gnn")
    ...
    for i in range(LOOK_BACK):
        ...
        df['addr_past_%d'%(i+1)]=df['block_address'].shift(periods=(i+1))
    ...
    df['graph']=df.apply(lambda x: build_graph(x['past_addr_abs'],x['past_page_abs'],x['past'],x['future']),axis=1)
    
    print("preprocessing done")
    return df[['id', 'cycle', 'addr', 'ip', 'graph','future']]
